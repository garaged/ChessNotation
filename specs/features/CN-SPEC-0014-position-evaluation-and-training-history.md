...
- CN-SPEC-0014-FR003: Before the first evaluated move is completed, the notation game screen must omit the evaluation or show an unavailable state without implying an even position.
...
- CN-SPEC-0014-AC002: Given a notation game has no stored evaluation for the current visible position, when the game screen renders, then no fabricated `0.0` evaluation is shown.
