...
Home and Board Skills rendered screenshots were inspected from `docs/manual-testing/screenshots/home-board-skills/2026-08-23/`. Default Dynamic Type cases passed. Accessibility Dynamic Type captures exposed clipping and need to be regenerated after the layout fix.
...
- CN-SPEC-0026 and CN-SPEC-0027 remain `Proposed` because accessibility Dynamic Type screenshot evidence failed before the current layout fix.
...
| --- | --- | --- | --- |
| Home / families | Home, Board Skills, Instructions card, Settings toolbar | `RestoredHomeView.swift`, `HomeTileLayoutRegressionTests`, `HomeUITests`, inspected screenshots | Default screenshots pass; accessibility screenshot failed before current fix. |
...
| Board Skills | Board Skills family, Square Recognition setup/game/results/history, Piece Movement game/results | `RestoredHomeView.swift`, `SquareRecognitionViews.swift`, `PieceMovementFeature.swift`, Home UI tests, inspected screenshots | Default screenshots pass; accessibility screenshot failed before current fix. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| UIAUD-001 | Home / Board Skills | Device matrix | High | Release evidence | Required rendered screenshot matrix has default-size PASS evidence, but accessibility Dynamic Type screenshots failed before the current layout fix, so CN-SPEC-0026 and CN-SPEC-0027 cannot be accepted yet. | `docs/manual-testing/home-board-skills-responsive-validation.md`; `docs/manual-testing/screenshots/home-board-skills/2026-08-23/home-iphone-16-portrait-accessibility.png`; `docs/manual-testing/screenshots/home-board-skills/2026-08-23/board-skills-iphone-16-portrait-accessibility.png`. | CN-SPEC-0026, CN-SPEC-0027 | Regenerate and inspect Home and Board Skills accessibility screenshots after the layout fix. | Open | Pending |
...
| UIAUD-009 | Home / Board Skills | iPhone 16 accessibility Dynamic Type | High | Accessibility typography | Accessibility Dynamic Type screenshots clipped important Home hero text and Board Skills Quick Start/drill row text. | `home-iphone-16-portrait-accessibility.png` shows hero title/subtitle truncation; `board-skills-iphone-16-portrait-accessibility.png` shows Quick Start and drill row truncation. | CN-SPEC-0027 | Allow Home hero text and Board Skills rows to grow vertically at accessibility Dynamic Type; recapture screenshots to confirm. | Fixed pending screenshot validation | Pending |

## Category Notes

### Layout

Home and Board Skills have strong source and frame-test coverage. Default-size rendered screenshots passed across the requested device classes. Accessibility Dynamic Type captures failed before the current layout fix and must be recaptured. Other gameplay surfaces are scroll-based and mostly use semantic fonts and system controls, but rendered iPad and accessibility Dynamic Type review is still required before release.
...
Existing tests cover board-square semantics, Dynamic Type source guardrails, Reduce Motion feedback, and several UI flows. The Home/Board Skills accessibility screenshots found real clipping in fixed-height premium surfaces; the current fix removes those fixed-height caps at accessibility sizes and needs screenshot confirmation. The audit did not find hidden-answer leakage in Position Recall accessibility labels by source inspection; recall tests also cover study/answer state semantics.

## Next Recommended Fixes

1. Regenerate and inspect the Home and Board Skills accessibility Dynamic Type screenshots after the current layout fix.
